...
test('Inject form and handle unlock buttons on inventory page', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    const card = document.querySelector(`div.vehicle-card[data-vin="${currentCardUuid}"]`);
      let carPrice = '';

if (card) {
  const priceContainer = card.querySelector('.featuredPrice');
  if (priceContainer) {
    const amountSpan = priceContainer.querySelector('.vehiclePricingHighlightAmount');

    if (amountSpan) {
      // Temporarily show it if it's hidden
      const prevDisplay = amountSpan.style.display;
      amountSpan.style.display = 'inline';

      // Use innerText (more reliable than textContent on hidden elements)
      carPrice = amountSpan.innerText.trim();

      // Restore original display
      amountSpan.style.display = prevDisplay;
    }
  }
}
